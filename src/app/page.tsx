...
type ViewType = 'overview' | 'monthly' | 'quarterly' | 'annual' | 'caixinhas'
...
export default function Dashboard() {
  ...
  const handleAddManual = async () => {
    if (!isAdmin) {
      alert('Acesso administrativo necessário para lançamentos manuais.');
      return;
    }
    const desc = prompt('Descrição do lançamento:');
    if (!desc) return;
    const value = prompt('Valor (positivo para entrada, negativo para saída):');
    if (!value) return;
  ...
  const filteredTransactions = useMemo(() => {
    return transactions.filter(t => {
      ...
      if (activeTab === 'monthly') return matchesSearch && periodKey === selectedPeriod
  ...
  const stats = useMemo(() => {
    ...
    if (activeTab === 'overview' || activeTab === 'caixinhas' || selectedPeriod === 'all') {
      ...
      const periodTransactions = transactions.filter(t => {
        ...
        if (activeTab === 'monthly') return `${y}-${m}` <= selectedPeriod
        if (activeTab === 'quarterly') {
  ...
  const periods = useMemo(() => {
    ...
    transactions.forEach(t => {
      ...
      const m = (d.getUTCMonth() + 1).toString().padStart(2, '0')
      if (activeTab === 'monthly') set.add(`${y}-${m}`)
      ...
      <div className="flex flex-col md:flex-row min-h-screen">
        <aside className="w-full md:w-64 bg-white dark:bg-zinc-900 border-r border-zinc-200 dark:border-zinc-800 p-6 flex flex-col">
          ...
          <nav className="space-y-1 mb-8">
            ...
            <button onClick={() => { setActiveTab('monthly'); setSelectedPeriod(periods[0] || 'all'); }} className={`w-full flex items-center gap-3 px-4 py-2 rounded-xl text-sm font-semibold transition-all ${activeTab === 'monthly' ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/30' : 'text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800'}`}>
          ...
          <div className="mt-auto space-y-3">
            ...
            {isAdmin && (
              <button onClick={handleAddManual} className="w-full flex items-center justify-center gap-2 bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 py-2 rounded-xl text-xs font-bold hover:opacity-90">
            ...
            <button onClick={() => fileInputRef.current?.click()} disabled={uploading} className="w-full flex items-center justify-center gap-2 bg-emerald-600 text-white py-3 rounded-xl text-sm font-bold hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-500/20">
              <Upload size={18} /> {uploading ? 'Aguarde...' : 'Adicionar Extrato'}
            </button>
            <input type="file" ref={fileInputRef} onChange={async (e) => {
               const files = e.target.files; if (!files) return;
               setUploading(true); const fd = new FormData(); for(let i=0; i<files.length; i++) fd.append('files', files[i]);
               const res = await fetch('/api/upload-pdf', { method: 'POST', body: fd });
               if((await res.json()).success) { alert('Dados adicionados com sucesso!'); fetchData(); }
               setUploading(false);
        ...
        <main className="flex-1 p-4 md:p-10 overflow-auto">
            ...
            <div>
              <h2 className="text-3xl font-black tracking-tight">
                {activeTab === 'overview' ? 'Panorama Geral' : 
                 ...
                 activeTab === 'monthly' ? 'Balanço Mensal' :
                 activeTab === 'quarterly' ? 'Balanço Trimestral' : 'Balanço Anual'}
              </h2>
              <p className="text-zinc-500 font-medium">
                {activeTab === 'caixinhas' ? 'Valores reservados por departamento' : 
                 activeTab === 'overview' ? 'Consolidado da gestão' : `Período: ${selectedPeriod}`}
              </p>
            </div>

            <div className="flex items-center gap-3">
              {['monthly', 'quarterly', 'annual'].includes(activeTab) && (
                <select className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl px-4 py-2 text-sm font-bold outline-none" value={selectedPeriod} onChange={(e) => setSelectedPeriod(e.target.value)}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-10">
            <div className="bg-white dark:bg-zinc-900 p-5 rounded-2xl border border-zinc-100 dark:border-zinc-800 shadow-sm">
              <p className="text-[10px] font-black text-zinc-400 uppercase tracking-widest mb-2">Caixa Inicial</p>
              <p className="text-xl font-bold text-blue-600">{formatCurrency(stats.initialBalance)}</p>
            </div>
            <div className="bg-white dark:bg-zinc-900 p-5 rounded-2xl border border-zinc-100 dark:border-zinc-800 shadow-sm">
              <p className="text-[10px] font-black text-emerald-500/70 uppercase tracking-widest mb-2">Entradas</p>
              <p className="text-xl font-bold text-emerald-600">{formatCurrency(stats.income)}</p>
            </div>
            <div className="bg-white dark:bg-zinc-900 p-5 rounded-2xl border border-zinc-100 dark:border-zinc-800 shadow-sm">
              <p className="text-[10px] font-black text-rose-500/70 uppercase tracking-widest mb-2">Saídas</p>
              <p className="text-xl font-bold text-rose-600">{formatCurrency(stats.expenses)}</p>
            </div>
            <div className="bg-white dark:bg-zinc-900 p-5 rounded-2xl border border-zinc-100 dark:border-zinc-800 shadow-sm">
              <p className="text-[10px] font-black text-purple-500/70 uppercase tracking-widest mb-2">Rendimentos</p>
              <p className="text-xl font-bold text-purple-600">{formatCurrency(stats.rendimentos)}</p>
            </div>
            <div className="bg-blue-600 p-5 rounded-2xl shadow-xl shadow-blue-500/20 lg:scale-105">
              <p className="text-[10px] font-black text-blue-200 uppercase tracking-widest mb-2">Saldo em Conta</p>
              <p className="text-2xl font-black text-white">{formatCurrency(stats.balance)}</p>
            </div>
          </div>
            ...
            {activeTab === 'caixinhas' ? (
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 space-y-6">
                    ...
                    {caixinhasData.map(c => (
                      <div key={c.name} className="bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-zinc-100 dark:border-zinc-800 shadow-sm flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-xl font-black text-zinc-800 dark:text-zinc-100">{formatCurrency(c.value)}</p>
                        </div>
                  ...
                  <div className="bg-zinc-50 dark:bg-zinc-900/50 p-6 rounded-3xl border-2 border-dashed border-zinc-200 dark:border-zinc-800">
                    <h4 className="font-bold text-sm mb-4 flex items-center gap-2 text-zinc-500 uppercase"><ArrowRightLeft size={16}/> Resumo de Movimentações</h4>
                    <p className="text-xs text-zinc-400 leading-relaxed italic">
                      Os valores acima são calculados automaticamente a partir dos lançamentos de "Dinheiro reservado" e "Dinheiro retirado" nos extratos. Para ajustar um saldo manualmente, utilize o Modo Admin.
                    </p>
                  </div>
                </div>

                <div className="bg-white dark:bg-zinc-900 p-8 rounded-3xl border border-zinc-100 dark:border-zinc-800 shadow-sm">
                  <h3 className="font-bold mb-6">Distribuição</h3>
              ...
              <>
                {activeTab === 'overview' && (
                  <div className="bg-white dark:bg-zinc-900 p-8 rounded-3xl border border-zinc-100 dark:border-zinc-800 shadow-sm">
                    <h3 className="font-bold text-lg mb-8">Evolução do Saldo</h3>
                ...
                <div className="bg-white dark:bg-zinc-900 rounded-3xl border border-zinc-100 dark:border-zinc-800 shadow-sm overflow-hidden">
                  <div className="p-6 flex items-center justify-between border-b border-zinc-50 dark:border-zinc-800">
                    <div className="relative w-full max-w-sm">
                      ...
                      <input type="text" placeholder="Pesquisar..." className="w-full bg-zinc-50 dark:bg-zinc-800 border-none rounded-2xl pl-12 pr-6 py-3 text-sm outline-none ring-1 ring-zinc-200 dark:ring-zinc-700 focus:ring-2 focus:ring-blue-500 transition-all" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                    </div>
                  </div>
                      ...
                      <tbody className="divide-y divide-zinc-50 dark:divide-zinc-800">
                        {filteredTransactions.map(t => (
                          <tr key={t.id} className="hover:bg-zinc-50/80 dark:hover:bg-zinc-800/50 transition-colors">
                            ...
                            <td className={`px-8 py-5 text-right font-black text-base ${t.amount > 0 ? 'text-emerald-500' : 'text-rose-500'}`}>
